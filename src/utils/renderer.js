...
async function initializeLocal(profile = 'interview') {
    ...
    if (success) {
        jarvis.setStatus('Local AI Live');
        return true;
    } else {
        jarvis.setStatus('error');
        return false;
...
ipcRenderer.on('update-status', (event, status) => {
    ...
    jarvis.setStatus(status);
});

// Shift+S — start session from shortcut
ipcRenderer.on('shortcut-start-session', () => {
    const app = document.querySelector('jarvis-app');
    if (app && !app.sessionActive) {
        console.log('Shortcut: starting session');
        app.handleStart();
    }
});

// Shift+A — stop session from shortcut
ipcRenderer.on('shortcut-stop-session', () => {
    const app = document.querySelector('jarvis-app');
    if (app && app.sessionActive) {
        console.log('Shortcut: stopping session');
        app.handleClose();
    }
});
...
const MANUAL_SCREENSHOT_PROMPT = `Answer the question on screen. Start directly with your answer — do not repeat or copy any text from the screen.`;
        ...
        async blob => {
            ...
            reader.onloadend = async () => {
                ...
                if (result.success) {
                    ...
                    jarvis.addNewResponse(`Error: ${result.error}`);
                }
            };
            reader.readAsDataURL(blob);
        },
...
function handleShortcut(shortcutKey) {
    const currentView = jarvis.getCurrentView();

    if (shortcutKey === 'ctrl+enter' || shortcutKey === 'cmd+enter') {
        if (currentView === 'main') {
            jarvis.element().handleStart();
        } else {
            // If text input has content, send it — otherwise take screenshot
            const assistantEl = jarvis.element()?.shadowRoot?.querySelector('jarvis-assistant-view');
            const textInput = assistantEl?.shadowRoot?.querySelector('#textInput');
            if (textInput && textInput.value.trim()) {
                assistantEl.handleSendText();
            } else {
...
const jarvisApp = document.querySelector('jarvis-app');
...
// Consolidated jarvis object - all functions in one place
const jarvis = {
    // App version
    ...
    getCurrentView: () => jarvisApp.currentView,
    getLayoutMode: () => jarvisApp.layoutMode,

    // Status and response functions
    setStatus: text => jarvisApp.setStatus(text),
    addNewResponse: response => jarvisApp.addNewResponse(response),
    updateCurrentResponse: response => jarvisApp.updateCurrentResponse(response),
