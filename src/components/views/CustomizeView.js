...
export class CustomizeView extends LitElement {
    ...
    getThemes() {
        return jarvis.theme.getAll();
    }

    async _loadFromStorage() {
        try {
            const [prefs, keybinds] = await Promise.all([jarvis.storage.getPreferences(), jarvis.storage.getKeybinds()]);
    ...
    getProfiles() {
        return [
            ...
            { value: 'customer', name: 'Customer P1/O1' },
            { value: 'tse', name: 'TSE Support' },
        ];
    }
    ...
    getDefaultKeybinds() {
        const isMac = jarvis.isMacOS || navigator.platform.includes('Mac');
        return {
    ...
    async saveKeybinds() {
        await jarvis.storage.setKeybinds(this.keybinds);
        if (window.require) {
    ...
    async handleCustomPromptInput(e) {
        ...
        await jarvis.storage.updatePreference('customPrompt', this.customPrompt);
    }

    async handleAudioModeSelect(e) {
        this.audioMode = e.target.value;
        await jarvis.storage.updatePreference('audioMode', this.audioMode);
        this.requestUpdate();
    }

    async handleThemeChange(e) {
        this.theme = e.target.value;
        await jarvis.theme.save(this.theme);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }
    ...
    async handleGoogleSearchChange(e) {
        ...
        await jarvis.storage.updatePreference('googleSearchEnabled', this.googleSearchEnabled);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleBackgroundTransparencyChange(e) {
        ...
        await jarvis.storage.updatePreference('backgroundTransparency', this.backgroundTransparency);
        this.updateBackgroundAppearance();
        this.requestUpdate();
    }

    updateBackgroundAppearance() {
        const colors = jarvis.theme.get(this.theme);
        jarvis.theme.applyBackgrounds(colors.background, this.backgroundTransparency);
    }

    async handleFontSizeChange(e) {
        this.fontSize = parseInt(e.target.value, 10);
        await jarvis.storage.updatePreference('fontSize', this.fontSize);
        this.updateFontSize();
        this.requestUpdate();
    }
    ...
    async resetKeybinds() {
        this.keybinds = this.getDefaultKeybinds();
        await jarvis.storage.setKeybinds(null);
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.send('update-keybinds', this.keybinds);
        }
    ...
    async restoreAllSettings() {
        ...
        try {
            ...
            for (const [key, value] of Object.entries(defaults)) {
                await jarvis.storage.updatePreference(key, value);
            }
            ...
            this.updateBackgroundAppearance();
            this.updateFontSize();
            await jarvis.theme.save(defaults.theme);
