...
export class AssistantView extends LitElement {
    ...
    connectedCallback() {
        ...
        if (window.require) {
            ...
            this.handleClipboardChanged = (event, text) => {
            ...
            ipcRenderer.on('clipboard-text-changed', this.handleClipboardChanged);
        }

        this._lastCtrlVTime = 0;
    }
    ...
    disconnectedCallback() {
        ...
        if (window.require) {
            ...
            if (this.handleClipboardChanged) ipcRenderer.removeListener('clipboard-text-changed', this.handleClipboardChanged);
        }
    }
    ...
    handleTextKeydown(e) {
        ...
        // Double Ctrl+V within 600ms = send
        if ((e.ctrlKey || e.metaKey) && e.key === 'v') {
            const now = Date.now();
            if (now - this._lastCtrlVTime < 600) {
                e.preventDefault();
                this._lastCtrlVTime = 0;
                this.handleSendText();
            } else {
                this._lastCtrlVTime = now;
            }
        }
    }
