...
export class JarvisApp extends LitElement {
    static styles = css`
        * {
        ...
        .log-type-session { color: var(--success); }
        .log-type-status { color: var(--text-secondary); }
        .log-type-response { color: var(--accent); }
        ...
        .live-bar-log-btn:hover, .live-bar-log-btn.active {
            color: var(--text-primary);
            background: var(--bg-hover);
        }
    ...
    static properties = {
        ...
        _logs: { state: true },
        _showLogs: { state: true },
        _proModel: { state: true },
        _planType: { state: true },
    };
    ...
    async _checkForUpdates() {
        try {
            this._localVersion = await jarvis.getVersion();
            this.requestUpdate();

            const res = await fetch('https://raw.githubusercontent.com/sohzm/jarvis/refs/heads/master/package.json');
            if (!res.ok) return;
    ...
    async _loadFromStorage() {
        try {
            ...
            this._planType = prefs.planType || 'welcome';
            this._proModel = prefs.proModel || 'gemini';
    ...
    _addLog(type, message) {
        ...
        this._logs = [...this._logs.slice(-99), { ts, type, message }];
    }
    ...
    async handleStart() {
        const prefs = await jarvis.storage.getPreferences();
        const providerMode = prefs.providerMode || 'cloud';

        if (providerMode === 'cloud') {
            const creds = await jarvis.storage.getCredentials();
            ...
            const success = await jarvis.initializeCloud(this.selectedProfile);
            if (!success) {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            const success = await jarvis.initializeLocal(this.selectedProfile);
            if (!success) {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            const apiKey = await jarvis.storage.getApiKey();
            if (!apiKey || apiKey === '') {
                const mainView = this.shadowRoot.querySelector('main-view');
                if (mainView && mainView.triggerApiKeyError) {
                    mainView.triggerApiKeyError();
            ...
            await jarvis.initializeGemini(this.selectedProfile, this.selectedLanguage);
        }

        jarvis.startCapture(this.selectedScreenshotInterval, this.selectedImageQuality);
        this.responses = [];
        this.currentResponseIndex = -1;
        ...
        this._logs = [];
        this._showLogs = false;
        this._addLog('session', `Session started — profile: ${this.selectedProfile}`);
        this._startTimer();

        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.invoke('increment-session-count').catch(() => {});
            ipcRenderer.invoke('get-pro-model').then(r => { this._proModel = r.model || 'gemini'; }).catch(() => {});
        }
    }
    ...
    async handleProfileChange(profile) {
        ...
        await jarvis.storage.updatePreference('selectedProfile', profile);
    }

    async handleLanguageChange(language) {
        this.selectedLanguage = language;
        await jarvis.storage.updatePreference('selectedLanguage', language);
    }

    async handleScreenshotIntervalChange(interval) {
        this.selectedScreenshotInterval = interval;
        await jarvis.storage.updatePreference('selectedScreenshotInterval', interval);
    }

    async handleImageQualityChange(quality) {
        this.selectedImageQuality = quality;
        await jarvis.storage.updatePreference('selectedImageQuality', quality);
    }

    async handleLayoutModeChange(layoutMode) {
        this.layoutMode = layoutMode;
        await jarvis.storage.updateConfig('layout', layoutMode);
        if (window.require) {
            try {
                const { ipcRenderer } = window.require('electron');
    ...
    async handleSendText(message) {
        const result = await window.jarvis.sendTextMessage(message);
        if (!result.success) {
    ...
    renderLiveBar() {
        ...
        const modelLabels = { gemini: 'Gemini 2.5', claude: 'Claude', gpt4o: 'GPT-4o' };
        ...
        return html`
            <div class="live-bar">
                ...
                <div class="live-bar-center">
                    ...
                    ${this._planType === 'plan' ? html`<span style="font-size:10px;background:rgba(59,130,246,0.12);color:var(--accent);border:1px solid rgba(59,130,246,0.25);border-radius:4px;padding:1px 6px;font-family:var(--font-mono)">${modelLabels[this._proModel] || this._proModel}</span>` : ''}
                    ${this.responses.length > 0 ? html`<span class="q-counter">Q&nbsp;${this.currentResponseIndex + 1}&nbsp;/&nbsp;${this.responses.length}</span>` : ''}
                </div>
                <div class="live-bar-right">
                    ...
                    <button class="live-bar-log-btn ${this._showLogs ? 'active' : ''}" @click=${() => { this._showLogs = !this._showLogs; }}>logs</button>
                    <span class="live-bar-text clickable" @click=${() => this.handleHideToggle()}>[hide]</span>
                </div>
            </div>
            ${this._showLogs ? html`
                <div class="logs-panel">
                    ${this._logs.length === 0 ? html`<span style="color:var(--text-muted)">No logs yet</span>` : ''}
