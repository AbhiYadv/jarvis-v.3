...
function getOpenaiApiKey() {
    // support both openaiApiKey (new) and openaiKey (legacy field saved by old UI)
    const creds = getCredentials();
    return creds.openaiApiKey || creds.openaiKey || '';
}
...
// ============ PRO LICENSE ============
const PRO_TEST_TOKENS = new Set(['JARVIS-PRO-TEST-0001', 'JARVIS-PRO-DEMO']);

function validateLicenseToken(token) {
    if (!token || !token.trim()) return false;
    const t = token.trim().toUpperCase();
    // Local test tokens always valid
    if (PRO_TEST_TOKENS.has(t)) return true;
    // Any non-empty token is treated as potentially valid (cloud validates it)
    return token.trim().length >= 8;
}

function getProLicense() {
    const creds = readJsonFile(getCredentialsPath(), DEFAULT_CREDENTIALS);
    return creds.proLicense || null;
}

function setProLicense(data) {
    const creds = readJsonFile(getCredentialsPath(), DEFAULT_CREDENTIALS);
    creds.proLicense = data;
    writeJsonFile(getCredentialsPath(), creds);
}

function getProModel() {
    const prefs = readJsonFile(getPreferencesPath(), DEFAULT_PREFERENCES);
    return prefs.proModel || 'gemini';
}

function setProModel(model) {
    const prefs = readJsonFile(getPreferencesPath(), DEFAULT_PREFERENCES);
    prefs.proModel = model;
    writeJsonFile(getPreferencesPath(), prefs);
}

function getDailySessionCount() {
    const limits = readJsonFile(getLimitsPath(), DEFAULT_LIMITS);
    const today = new Date().toISOString().slice(0, 10);
    const entry = limits.data.find(d => d.date === today);
...
function incrementDailySessionCount() {
    const limits = readJsonFile(getLimitsPath(), DEFAULT_LIMITS);
    const today = new Date().toISOString().slice(0, 10);
    let entry = limits.data.find(d => d.date === today);
    if (!entry) {
        entry = { date: today, flash: { count: 0 }, flashLite: { count: 0 }, sessionCount: 0 };
